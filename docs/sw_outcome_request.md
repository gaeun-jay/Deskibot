...
1. 읽기 전용 뷰 하나 적용 (DDL 한 방, 코드 변경 없음)
...
이름만 붙이는 읽기 전용 뷰**를 준비했습니다.

```
HW 레포 esp 브랜치 → server/sql/004_focus_outcome_view.sql
```

`database/` 시퀀스의 `004`로 옮겨서 적용해 주시면 됩니다. 뷰 하나 추가라
기존 테이블·제약·앱 어느 쪽도 건드리지 않습니다.
...
- **SQL은 실행해 보지 못했습니다.** HW 쪽에 postgres가 없어서 문법만 기존
  `002` / `003` 마이그레이션 스타일에 맞췄습니다. 적용하실 때 처음 돌아갑니다.
