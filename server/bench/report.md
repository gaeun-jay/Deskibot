...
| 유형 | clova_csr | google_v1 |
|---|---|---|
| 단일 요청 (60건) | 93% (56/60) | 93% (56/60) — **동률** |
| **복합 요청 (30건)** | **90%** (27/30) | **73%** (22/30) |
...
| CLOVA CSR | **15초 올림** | 없음 | 콘솔 확인 필요 (VAT 별도)<br>호출당 최대 60초 |
...
계산 재현: `python3 bench/cost.py --calls-per-day 20`
(CLOVA 단가를 알면 `--clova-per-15s <원>`을 붙여 총액 비교까지 나온다.)
...
| **복합 발화** | **CLOVA** | **90% vs 73% — 가장 뚜렷한 차이** |
| 단일 발화 | 동률 | 93% vs 93% |
...
| 사용량 | Google | CLOVA 손익분기 단가 |
|---|---|---|
| 하루 20회 (월 600) | **0원** (무료 한도 내) | — (단가 무관 Google 우세) |
| 하루 100회 (월 3,000) | 6,714원 | 15초당 2.24원 |
| 하루 500회 (월 15,000) | 41,346원 | 15초당 2.76원 |

CLOVA 15초당 단가가 위 손익분기보다 비싸면 Google이, 싸면 CLOVA가 저렴하다.
(CLOVA 실단가는 콘솔 요금표 확인 필요 — 공개 페이지가 JS로 API를 호출해 채우는
구조라 확보하지 못했다. 추정치를 넣으면 판단을 왜곡하므로 비워 둔다.)

**단가 결론: Google 우세.** 근거는 단가가 아니라 **과금 구조**다.
...
- **비용**: 현재 사용량에서 Google은 무료(월 673회 한도), CLOVA는 유료다.
  월 사용량이 늘면 재검토가 필요하다.
...
- 월 사용량이 673회(하루 22회)를 넘어 Google도 유료가 되면 비용 비교를 다시 한다
...
- CLOVA 실단가 미확보 — 단가 결론은 과금 구조와 무료 한도에 근거한 것이다.
